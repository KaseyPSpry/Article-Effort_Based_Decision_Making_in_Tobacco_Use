...
data {
  ...
  array[num_subjects] int<lower=0, upper=max_trials> trials_per_subj; // define the number of trials per subject
  array[num_subjects, max_trials] real<lower=-1> choiceRT; // define the reaction time
  array[num_subjects, max_trials] int<lower=-1, upper=2> choice; // define whether on each trial the subject chose a hard option or not (decision on each trial, yes or no)
  array[num_subjects, max_trials] real<lower=-1, upper=1> probability; // define the probability of reward at each trial
  array[num_subjects, max_trials] real<lower=-1> reward;  // define the magnitude of reward on each trial
...
parameters {
  ...
  vector[num_subjects] h_pr; // h: Weight of probability
  vector[num_subjects] k_pr; // k: Cost of effort
  vector[num_subjects] inv_temp_pr; // inverse temperature for softmax decision rule
}

transformed parameters {

  vector<lower=0, upper=10>[num_subjects] h;
  vector<lower=0, upper=10>[num_subjects] k;
  vector<lower=0, upper=100>[num_subjects] inv_temp;

  for (i in 1:num_subjects) {
    int g = group_id[i];
    h[i] = Phi_approx(mu_pr_group[g, 1] + sigma_pr[1] * h_pr[i]) *10;
    k[i] = Phi_approx(mu_pr_group[g, 2] + sigma_pr[2] * k_pr[i]) *10;
    inv_temp[i] = Phi_approx(mu_pr_group[g, 3] + sigma_pr[3] * inv_temp_pr[i]) *100;
  }
      ...
      // trial loop
       for (trial in 1:(trials_per_subj[i])) {
         ...
         if (choice[i, trial] > 0) {
          ...
          SVHard = reward[i,trial] * pow(probability[i,trial], h[i]) - (k[i] * 1.0);
          ...
          SVEasy = 1.0 * pow(probability[i,trial], h[i]) - (k[i] * 0.3);
...
generated quantities {
  ...
  for (g in 1:4) {
    mu_h_group[g] = Phi_approx(mu_pr_group[g, 1])*10;
    mu_k_group[g] = Phi_approx(mu_pr_group[g, 2])*10;
